...
func _init_audios() -> void:
	for a in audios:
		...
		resource_dictionary[a.audio_name] = a
...
func play_audio(_audio_name: String) -> void:
	...
	var resource = resource_dictionary.get(_audio_name, null) as AudioMangerResource
	audio.pitch_scale = resource.pitch_scale + randf_range(resource.random_pitch_offset.x,resource.random_pitch_offset.y)
